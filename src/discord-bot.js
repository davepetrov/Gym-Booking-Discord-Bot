...
function setConfig(
    ...
    if (!isUser(message)) {
        ...
        // add new entry
        db.prepare(
            `INSERT INTO ${dbName} (id, email, password, location, locationBackup, begin, end) VALUES ('${id}', '${email}', '${password}', '${location}', '${locationBackup}', '${begin}', '${end}')`
        ).run();
        ...
        const msg = new MessageEmbed()
            ...
            .setColor(0x61bf33)
            .setDescription(
                "**Creating** new config for you. *You can now use !book to book*"
            );
        message.author.send(msg);
    } else {
        ...
        // update the config
        db.prepare(
            `UPDATE ${dbName} SET email='${email}', password='${password}', location='${location}', locationBackup='${locationBackup}', begin='${begin}', end='${end}' WHERE id='${id}'`
        ).run();
        ...
        const msg = new MessageEmbed()
            ...
            .setColor(0x009cdf)
            .setDescription(
                "**Updating** your existing config. *You can now use !book to book*"
            );
        message.author.send(msg);
    }
